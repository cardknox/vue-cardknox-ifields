...
const readyToSubmitToGateway = computed(() => {
  return (
    cardData.value.cardToken.length > 0 && cardData.value.cvvToken.length > 0 && amount.value > 0
  )
})

async function handle3DSResults(
  actionCode,
  xCavv,
  xEciFlag,
  xRefNum,
  ...
  try {
    ...
    const response = await fetch('/api/verifyjson', {
      method: 'POST',
...
function onUpdate({ data }) {
  if (data && data.issuer) issuer.value = data.issuer
}
function onSubmit() {
...
async function submitToGateway() {
  let request = {
    ...
    xExp: format(new Date(cardData.value.year, cardData.value.month - 1), 'MMyy'),
          ...
          <div class="column">
            <section class="box card-box">
              ...
              <o-field label="Expiration">
                <div>
                  <o-select :placeholder="cardData.month">
                    <option :value="i" v-for="i in 12" :key="i" aria-role="listitem">
                      {{ i }}
                    </option>
                  </o-select>
              ...
              <div class="button-spaced">
                <o-button rounded :disabled="!readyToSubmitToGateway" @click="submitToGateway"
                  >Submit To Gateway</o-button
                >
              </div>
            </section>
          </div>
